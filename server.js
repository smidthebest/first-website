    ...
    pool.getConnection(function(err, con) {
        ...
        con.query("SELECT * FROM users WHERE email = '" + email + "'", function(err, result) {
            ...
            if (result.length == 0) {
                ...
                finEmail = email;
                con.query("CREATE TABLE mydb." + finEmail + " (num int, name varchar(255), data longtext, dt DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, image mediumblob)", function(err, result) {
                    if (err) throw err;
